...
describe('ActiveDirectoryAuthPlugin', () => {
    ...
    describe('authenticate', () => {
        it('usernameで認証できる', async () => {
            ...
            expect(bindMock).toHaveBeenCalledWith('testuser@example.local', 'password');
        ...
        it('usernameが未指定の場合は例外', async () => {
            ...
            ).rejects.toThrow('ユーザー名（またはメールアドレス）とパスワードを入力してください。');
        });
        ...
        it('LDAP_URL未設定の場合は例外', async () => {
            ...
            ).rejects.toThrow('LDAP_URL または LDAP_DOMAIN が設定されていません。');
        });
        ...
        it('bind失敗時は認証失敗エラーを返す', async () => {
            ...
            bindMock.mockRejectedValue(new Error('LDAP Error'));
            ...
            ).rejects.toThrow('Active Directory authentication failed');
        ...
        it('loginIdにメールアドレスを指定した場合はUPNへ変換する', async () => {
            ...
            expect(bindMock).toHaveBeenCalledWith('john@example.local', 'password');
        });

        it('unbind失敗は無視する', async () => {
            const plugin = new ActiveDirectoryAuthPlugin();

            unbindMock.mockRejectedValue(new Error('unbind failed'));
