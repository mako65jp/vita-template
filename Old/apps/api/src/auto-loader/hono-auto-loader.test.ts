...
vi.mock('@shared/server-utils', () => ({
    ...
    resolveFromProjectRoot: vi.fn((...paths: string[]) => `/project/${paths.join('/')}`),
}));

function createPlugin(overrides: Partial<ServerPluginManifest> = {}): ServerPluginManifest {
    return {
        id: 'users',
...
describe('loadFeatureModules', () => {
    ...
    describe('plugin mount', () => {
        it('有効なプラグインをマウントできる', async () => {
            ...
            expect(app.use).toHaveBeenCalledWith('/api/users/*', expect.anything());

            expect(app.route).toHaveBeenCalledWith('/api/users', plugin.routes);
        });
        ...
        it('requiredRole がある場合は RBAC を適用する', async () => {
            ...
            expect(rbacMiddleware).toHaveBeenCalledWith(['admin']);
    ...
    describe('middleware', () => {
        it('authMiddleware に JWT_SECRET を渡す', async () => {
            ...
            expect(authMiddleware).toHaveBeenCalledWith('test-secret');
        });
    });
    ...
    describe('plugin status', () => {
        it('getActivePlugins に DB を渡す', async () => {
            ...
            expect(getActivePlugins).toHaveBeenCalledWith(db);
        });
    });
    ...
    describe('module loading', () => {
        it('glob に pattern と rootDir を渡す', async () => {
            ...
            await loadFeatureModules(app, 'plugins/**/*.ts', db);

            expect(glob).toHaveBeenCalledWith('plugins/**/*.ts', {
                cwd: '/project',
            });
        });
        ...
        it('プラグインファイルが存在しなくても例外にならない', async () => {
            ...
            await expect(loadFeatureModules(app, '**/*.module.ts', db)).resolves.not.toThrow();
        });
    });
});
