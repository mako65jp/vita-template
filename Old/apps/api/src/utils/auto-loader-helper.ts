...
export async function getActivePlugins(db: Database) {
    ...
    try {
        ...
        console.warn(
            '[Plugin Helper] DB query failed or table not found. Defaulting all plugins to enabled.',
        );
    }

    return pluginRegistry.getAll().map((plugin: ServerPluginManifest) => {
        const isEnabled = dbPluginsMap.has(plugin.id) ? dbPluginsMap.get(plugin.id)! : true;
